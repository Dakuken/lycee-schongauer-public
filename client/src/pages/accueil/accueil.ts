...
class RecupActu {
    static actualites: ActualiteAccueil[] = [];

    static currentActu = 0;
    ...
    static async getActu(): Promise<void> {
        let req = await axios.get(`${API_LINK}/toute-actualite/actualite-accueil.php`).then((res) => {

            RecupActu.actualites = res.data.data;
        }).catch((e: AxiosError) => {
            if (e.response) {
                RecupActu.error = String(e.response.data);

            } else {
                RecupActu.error = "Une erreur s'est produite lors de la requête.";
            }
        });
...
class Accueil extends HTMLElement {
    ...
    showServices() {
        let servicesContainerWeb = <HTMLDivElement>document.querySelector("#brand-container-web");
        let servicesContainerMobile = <HTMLDivElement>document.querySelector("#brand-container-mobile");
        let html = ``;
        let htmlMobile = ``
        services.length = 5;
    ...
    showFormation() {
        let formationContainer = <HTMLDivElement>document.querySelector("#formation-container");
        let formationMobile = <HTMLDivElement>document.querySelector("#formation-mobile-container");
        let html = ``;
        let htmlMobile = ``;
    ...
    windowResize() {
        this.accueilPerso = <Accueil>document.querySelector("accueil-perso");
        this.accueilMobile = <AccueilMobile>document.querySelector("accueil-perso > accueil-mobile");
        this.accueilPc = <AccueilPc>document.querySelector("accueil-perso > accueil-pc");
    ...
    selectorCarroussel() {
        let titleWeb = <HTMLParagraphElement>document.querySelector("#titleWeb");
        let corpsWeb = <HTMLParagraphElement>document.querySelector("#corpsWeb");
        let imageWeb = <HTMLImageElement>document.querySelector(".imageWeb");
        let aWeb = <HTMLAnchorElement>document.querySelector("#actu-web-container_text");
        let titleMobile = <HTMLParagraphElement>document.querySelector("#titleMobile");
        let corpsMobile = <HTMLParagraphElement>document.querySelector("#corpsMobile");
        let imageMobile = <HTMLDivElement>document.querySelector(".imageMobile");
        let aMobile = <HTMLAnchorElement>document.querySelector("#actu-mobile-container_text");
