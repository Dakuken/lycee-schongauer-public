import "../../../snowpack/pkg/hammerjs.js";
    ...
    return new Promise(async (res, rej) => {
      ...
      if (token) {
        await axios.post(`${API_LINK}/client/info-client.php`, {token}).then((response) => {
          ...
          _UserData.nom = data.nom;
          _UserData.prenom = data.prenom;
    ...
    infoBox.addEventListener("click", () => {
      if (token) {
        if (infoBox.children.length > 1) {
          ...
          let info = document.createElement("info-perso");
...
export class NavBar extends HTMLElement {
  ...
  windowResize() {
    this.navbarPerso = document.querySelector("navbar-perso");
    this.navbarMobile = document.querySelector("navbar-perso > navbar-mobile");
    this.navbarPc = document.querySelector("navbar-perso > navbar-pc");
