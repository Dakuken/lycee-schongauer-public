...
const _DataXMLFromMBN = class {
        ...
        if (xhr.readyState === 4) {
          if (xhr.status === 200) {
            ...
            for (let i = 0; i < items.length; i++) {
              ...
              _DataXMLFromMBN.allArticle.push(tempArticles);
            }
            resolve();
          }
        }
  ...
  async get5Actu() {
    return new Promise(async (resolve) => {
      let nbArticle = _DataXMLFromMBN.displayArticle.length;
      let tempArticle = _DataXMLFromMBN.allArticle.slice(nbArticle, nbArticle + 5);
      ...
      _DataXMLFromMBN.displayArticle.push(...tempArticle);
      resolve();
    });
  }
...
let DataXMLFromMBN = _DataXMLFromMBN;
DataXMLFromMBN.displayArticle = [];
DataXMLFromMBN.allArticle = [];
DataXMLFromMBN.hasError = false;
...
class TouteActualite extends HTMLElement {
  constructor() {
    super();
    this.dataXML = new DataXMLFromMBN();
  }
  connectedCallback() {
    let size = window.innerWidth;
    const dataXML = new DataXMLFromMBN();
    dataXML.getActualite().then(() => {
      this.innerHTML = `
    ...
    let boutonLoad = document.querySelector("#bouton-load-actu");
  ...
  windowResize() {
    this.actualitePerso = document.querySelector("actualite-perso");
    this.actualiteMobile = document.querySelector("actualite-perso > actualite-mobile");
    this.actualitePc = document.querySelector("actualite-perso > actualite-pc");
